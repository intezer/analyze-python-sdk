"""
Paginated cases history result set.

This module exposes :class:`CasesHistoryResult`, returned from
:func:`intezer_sdk.cases.query_cases_history`, which lazily paginates over case
records that match a search filter.
"""
from intezer_sdk.api import IntezerApiClient
from intezer_sdk.api import raise_for_status
from intezer_sdk.history_results import HistoryResult


class CasesHistoryResult(HistoryResult):
    """Paginated result set of cases matching a search filter."""

    def __init__(self, request_url_path: str, api: IntezerApiClient, filters: dict):
        """
        Fetch all cases history results from server.
        """
        super().__init__(request_url_path, api, filters)

    def _fetch_history(self, url_path: str, data: dict) -> tuple[int, list]:
        """
        Request cases from server according to filters.
        :param url_path: Url to request new data from.
        :param data: Filters data.
        :return: Count of all results in filtered request and the cases themselves.
        """
        response = self._api.request_with_refresh_expired_access_token(
            path=url_path, method='POST', data=data)
        raise_for_status(response)
        data_response = response.json()['result']
        return data_response['cases_count'], data_response['cases']
